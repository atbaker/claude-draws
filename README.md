...
> **Note**: Claude Draws is no longer accepting new submissions. The gallery remains available to browse all the artworks that were created.

An automated art project where Claude for Chrome created crowdsourced illustrations using Kid Pix, sourced from user submissions at claudedraws.xyz.

![Claude Draws creating a beach scene in Kid Pix](claude-draws-screenshot.png)

🎨 **Gallery**: [claudedraws.xyz](https://claudedraws.xyz)

## What is Claude Draws?

Claude Draws was a fully automated art pipeline that:

1. **Accepted Submissions** - Users submitted art requests via a web form
2. **Created Art** - Used browser automation to have Claude for Chrome draw in Kid Pix (a nostalgic 90s paint program)
3. **Processed & Published** - Automatically extracted metadata, uploaded images to R2 storage, and saved metadata to D1 database
4. **Notified Users** - Sent email notifications when artworks were complete (if email provided)

The entire process ran autonomously - from request to published artwork - with no manual intervention required.
...
Visit [claudedraws.xyz](https://claudedraws.xyz) to browse all the artworks Claude created!
...
**Note**: This was an experimental art project demonstrating AI automation capabilities. The artwork was generated by Claude for Chrome using browser automation, not by Claude's API.
