<script lang="ts">
	...
	// State for autogenerated filter
	let showAutogenerated = $state(false);
	...
	// Filter artworks based on autogenerated toggle, and sort by createdAt descending
	let filteredArtworks = $derived(
		(showAutogenerated
			? data.artworks
			: data.artworks.filter((artwork) => !artwork.autogenerated)
		).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
	);

	// Artworks to display (with pagination)
	let displayedArtworks = $derived(filteredArtworks.slice(0, displayedCount));

	// Check if there are more items to load
	let hasMore = $derived(displayedCount < filteredArtworks.length);
	...
	$effect(() => {
		// This effect runs when showAutogenerated changes
		showAutogenerated;
		displayedCount = ITEMS_PER_PAGE;
	});
</script>
...
<div class="min-h-screen">
	...
	<main class="container mx-auto p-4 sm:p-8">
		<!-- Stats/Intro Section with Filter Controls -->
		<div class="bg-kidpix-yellow border-4 border-black p-6 shadow-chunky-lg mb-6">
			<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
				<p class="text-lg sm:text-xl font-bold text-center sm:text-left leading-relaxed">
					Claude Draws has created {data.artworks.length} artwork{data.artworks.length === 1
						? ''
						: 's'}.
				</p>
				<button
					onclick={() => (showAutogenerated = !showAutogenerated)}
					class="bg-kidpix-orange text-black font-bold text-lg px-6 py-3 border-4 border-black shadow-chunky hover:shadow-chunky-hover hover:translate-x-1 hover:translate-y-1 active:translate-x-2 active:translate-y-2 active:shadow-none uppercase transition-all whitespace-nowrap"
				>
					{showAutogenerated ? 'Show Only User Submissions' : 'Show All Artworks'}
				</button>
			</div>
		</div>
